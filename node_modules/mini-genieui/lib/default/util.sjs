...
const calcScrollLeft = (windowWidth, tabScrollPercentage, current) => {
  let scrollInit = current * windowWidth * tabScrollPercentage;

  if (current <= 3) {
    scrollInit = 0;
  } else {
    scrollInit = (current - 3) * windowWidth * tabScrollPercentage;
  }
